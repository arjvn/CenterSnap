...
class PanopticModel(pl.LightningModule):
  ...
  def configure_optimizers(self):
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lr_lambda)
        ...
        # Wrap the scheduler in a dictionary
    scheduler_config = {
        'scheduler': scheduler,
        'interval': 'epoch',  # or 'step' if you want the scheduler to update each step rather than each epoch
        'frequency': 1,  # How often the scheduler is applied. Default is every epoch.
        'reduce_on_plateau': False,  # For ReduceLROnPlateau scheduler, but good to specify to avoid confusion
        # 'monitor': 'metric_to_track',  # If your scheduler needs to track a metric, specify its name here
    }
